...
const Home = () => {
  ...
  const handleLocateMe = async () => {
    ...
    try {
      ...
      console.warn('Geolocation lookup:', err.message);
    } finally {
      setIsLocating(false);
    }
  };

  const displayLocation = userLocation?.name || user?.addressDetails?.city || user?.city || 'Chandigarh';

  // ══════════════════════════════════════════════════════════════════
  // POST-LOGIN MOBILE-FIRST VIEW (MATCHING IMAGE 2 EXACTLY)
  // ══════════════════════════════════════════════════════════════════
  if (user) {
    return (
      <div className="lf-signedin-container">
        {/* Top App Bar with Hamburger Icon */}
        <header className="lf-signedin-header">
          <div className="lf-signedin-brand">Localfixr</div>
          <button 
            type="button" 
            className="lf-signedin-menu-btn"
        ...
        <main className="lf-signedin-content">
          {/* Location Picker Row matching Image 2 */}
          <div className="lf-signedin-loc-row" onClick={handleLocateMe} title="Tap to update your location">
            <MapPin size={18} className="lf-signedin-loc-pin" />
            <span className="lf-signedin-loc-name">
              {isLocating ? 'Detecting GPS...' : displayLocation}
            </span>
          </div>

          {/* Search Bar matching Image 2 */}
          <form onSubmit={handleSearchSubmit} className="lf-signedin-search-form">
            <div className="lf-signedin-search-box">
              <Search size={18} className="lf-signedin-search-icon" />
              <input 
                type="text" 
                placeholder="Search for a service..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="lf-signedin-search-input"
              />
            </div>
          </form>
          ...
          <div 
            className="lf-signedin-hero-card"
            onClick={() => navigate('/search')}
            role="button"
            tabIndex={0}
          >
            <div className="lf-signedin-hero-overlay">
              <h2 className="lf-signedin-hero-title">
                Skilled Local<br />Professionals<br />Near You
              </h2>
              <p className="lf-signedin-hero-subtitle">
                Verified. Trusted. Ready to Help.
              </p>
            </div>
          </div>

          {/* Popular Services Section matching Image 2 */}
          <section className="lf-signedin-services-section">
            <div className="lf-signedin-services-head">
              <h3 className="lf-signedin-services-title">Popular Services</h3>
              <Link to="/search" className="lf-signedin-view-all">View all →</Link>
            </div>

            <div className="lf-signedin-services-grid">
              {popularServices.map((svc) => (
                <button
                  key={svc.id}
                  type="button"
                  className="lf-signedin-service-tile"
                  onClick={() => handleServiceClick(svc.categoryId)}
                >
                  <div className="lf-signedin-tile-icon">{svc.icon}</div>
                  <span className="lf-signedin-tile-name">{svc.name}</span>
                </button>
              ))}
            </div>
          </section>
        ...
        {/* Right Side Slide-Over Navigation Drawer (Image 1) */}
        <MobileSideDrawer 
  ...
  // ══════════════════════════════════════════════════════════════════
  // PRE-LOGIN VIEW (PERFECT SMARTPHONE ALIGNMENT, NO DOWNLOAD SECTION)
  // ══════════════════════════════════════════════════════════════════
  return (
    <div className="lf-home">

      {/* ═══ HERO — responsive background image & layout ═══ */}
